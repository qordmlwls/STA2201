...
model {
  // Log-likelihood
  target += normal_lpdf(log_weight | beta[1] + beta[2] * log_gest + beta[3]*prematurity + beta[4]*interac + beta[5]*log_bmi, sigma);
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_weight_hat_n = beta[1] + beta[2] * log_gest[n] + beta[3]*prematurity[n] + beta[4]*interac[n] + beta[5]*log_bmi[n];
